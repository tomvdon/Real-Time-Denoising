...
void scatterRay(
    ...
    if (m.microfacet) {
        ...
        auto albedo = glm::vec3(1, 0, 0); //hardcode surface color to grey;
        ...
        glm::vec3 brdf = fresnel * G * D / max(4.0f * NdotL * NdotV, 0.0001f);
        ...
        glm::vec3 color = Lo;
        color = (kD * glm::vec3(1, 0.0, 0.0) / 3.141592f + brdf) * radiance * NdotL;
        color = color / (color + glm::vec3(1));
